...
describe('CaixaDaLanchonete', () => {
    ...
    // Testes de compra com múltiplas quantidades
    test.each([
    ...
    test.each([
        ...
        ['com um valor', 'Item inválido!', ['1']],
        ['com código inexistente', 'Item inválido!', ['pizza,1']],
        ['com forma de pagamento inválida', 'Forma de pagamento inválida!', ['cafe,1']],
    ])('compra %p em %p deve resultar em %p', (_, formaDePagamento, resultadoEsperado, itens) =>
        validaTeste(formaDePagamento, resultadoEsperado, itens));

    // Testes de itens extras sem item principal
    test.each([
        ...
        ['chantily com outro item', 'Item extra não pode ser pedido sem o principal', ['chantily,1', 'sanduiche,1']],
        ['queijo com outro item', 'Item extra não pode ser pedido sem o principal', ['cafe,1', 'queijo,1']],
    ])('compra %p em %p deve resultar em %p', (_, formaDePagamento, resultadoEsperado, itens) =>
        validaTeste(formaDePagamento, resultadoEsperado, itens));
});
